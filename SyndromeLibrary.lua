...
-- require() / loadstring(HttpGet()) friendly.
...
-- Constructor
--========================
function Syndrome.new(options)
	local self = setmetatable({}, Syndrome)
...
--========================
function Syndrome:_track(conn)
	table.insert(self._connections, conn)
	return conn
end

function Syndrome:_disconnectAll()
...
function Syndrome:SetVisible(v)
	if self._mainGui then
		self._mainGui.Enabled = (v == true)
	end
end

function Syndrome:ToggleVisible()
	if self._mainGui then
...
-- CreateWindow
--========================
function Syndrome:CreateWindow()
	...
	if not player then
		error("SyndromeUI must be used from a LocalScript (client). Players.LocalPlayer is nil.")
	end
	...
	--==================================================
	local CONFIG_ATTR = "Syndrome_Config_JSON"
	...
	local function _getConfigPath()
		local fileName = (type(self.Options.ConfigFile) == "string" and self.Options.ConfigFile) or "SyndromeConfig.json"
	...
	local closeHint = mk("TextLabel", {
		...
		TextYAlignment = Enum.TextYAlignment.Top,
		TextWrapped = true,
		Text = "Are you sure you want to shut down Syndrome? (You can reopen with End)",
		Font = Enum.Font.Gotham,
		TextSize = 12,
		TextColor3 = Color3.fromRGB(220,220,235),
		TextTransparency = 0.30,
	...
	Notify("Syndrome", "press End to toggle UI", nil)
	return self
end
...
--   local Syndrome = loadstring(game:HttpGet(https://raw.githubusercontent.com/meekazure/Syndrome/main/Ui/SyndromeLibrary.lua))()
--
--   -- B) ModuleScript require 
--   -- Put this file as a ModuleScript, e.g. ReplicatedStorage/SyndromeUI_Premium_V1
--   local Syndrome = require(game:GetService("ReplicatedStorage"):WaitForChild("SyndromeUI_Premium_V1"))
--
--
-- 2) Create the UI window:
--
--   local ui = Syndrome.new({
--     AutoDemo = false,                   -- true = library builds demo tabs automatically
--     ConfigFile = "SyndromeConfig.json", -- (optional) file name if file IO exists
--     ConfigFolder = "SyndromeUI",        -- (optional) folder if file IO exists
--   }):CreateWindow()
--
--
-- 3) Tabs + Sections:
